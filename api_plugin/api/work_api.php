...
add_action( 'rest_api_init', function () {
  register_rest_route( 'work/v1', '/work_api', array(
    'methods' => 'POST',
...
/* 取得  Post By Title  */
add_action( 'rest_api_init', function () {
  register_rest_route( 'work/v1', '/get_post_by_title', array(
    'methods' => 'POST',
    'callback' => 'get_post_by_title_func',
  ) );
});
function get_post_by_title_func($data){
    ...
    if ( $the_query->have_posts() ) {
        while ( $the_query->have_posts() ) {
            ...
            $ori = get_post_meta( get_the_ID(), '_xmy_meta_value_key', true );
            $value  = ($ori)? json_decode($ori,true):'';
            
            $style = get_post_meta( get_the_ID(), '_wp_style', true );
